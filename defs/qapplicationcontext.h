...
template<typename S,ServiceInitializationPolicy serviceInitPolicy=ServiceInitializationPolicy::DEFAULT> struct default_service_traits {
...
template<typename Srv,typename Impl=Srv,ServiceScope scope=ServiceScope::UNKNOWN> struct Service {
     ...
     * @param initializationPolicy determines whether the *init-method* will be invoked before or after the service has been published.
     * @return this instance
     */
    template<typename I> Service<Srv,Impl,scope>&& withInit(I initializer, ServiceInitializationPolicy initializationPolicy = ServiceInitializationPolicy::DEFAULT) && {
     ...
     */
    template<typename I> Service<Srv,Impl,scope> withInit(I initializer, ServiceInitializationPolicy initializationPolicy = ServiceInitializationPolicy::DEFAULT) const& {
        return Service<Srv,Impl,scope>{*this}.withInit(initializer, initializationPolicy);
    }
