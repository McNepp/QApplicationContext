...
template<> struct service_traits<BaseService2> : default_service_traits<BaseService2,ServiceInitializationPolicy::AFTER_PUBLICATION> {
    using initializer_type = service_initializer<&BaseService2::init>;
};
...
void testWatchConfigurationFileChangeWithError() {
  ...
  void testAutoRefreshPlaceholderPropertyResolveError() {
    ...
    void testInitAfterPublication() {
        ...
        auto reg = context->registerService(service<QTimer>().withInit(static_cast<void(QTimer::*)()>(&QTimer::start), ServiceInitializationPolicy::AFTER_PUBLICATION));
